    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                pred = penergy.squeeze(-1)
                target = tenergies[i]
                #pred = self.scale * pred
                residual = (target - pred) / target
                resolution_loss = torch.mean(residual**2)
                mean_residual = torch.mean(residual)
                bias_loss = mean_residual
                lam = 0.1
                lenergy += resolution_loss # + lam * bias_loss
                
                
                #log_pred = torch.log(pred)
                #log_true = torch.log(target)
                #lenergy += torch.mean(((target - pred) / target) ** 2)
                #residual = (target - pred) / target
                #eps = 1e-8
                #lenergy += torch.mean(torch.log(residual**2 + eps))
                #resolution_loss = torch.mean(residual ** 2)
                #bias_penalty = torch.mean(residual) ** 2
                #lam = 0.01
                #lenergy += resolution_loss + lam * bias_penalty
